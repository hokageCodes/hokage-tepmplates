...
export default function WhatYouGet() {
  return (
    <section className="w-full mt-20">
      <h2 className="text-2xl font-extrabold flex items-center gap-2 mb-12 text-neutral-900">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12 w-full">
        {items.map((item) => (
          <div key={item.label} className="flex flex-col items-center text-center gap-3">
            <span className="rounded-full bg-accent/10 p-4 flex items-center justify-center">
              {item.icon}
            </span>
            <span className="font-semibold text-lg text-neutral-900 mt-2">{item.label}</span>
          </div>
        ))}
      </div>
    </section>
